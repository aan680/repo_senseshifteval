...
def synset_rank(word, synset): #what is the number of a given synset for a word? I.e. how central is the word to this synset? 
	...
	#return  which(synsets=="abominable.s.01") that is R syntax

def allinfo_synset(synset):
	info = {'synset':synset_name(synset), 'synset_def':synset_def(synset), 'ref':synset_words(synset), 'pos':pos_my_notation(synset), 'synset_size':synset_size(synset)}
	return info
...
def allinfo_word(target, pos):
	...
	#return [allinfo(x,y) for x in targets, y in synsets]
	#list(map(lambda x: x**2, items))


#synset = synset_pos = pos_my_notation(synset), 
